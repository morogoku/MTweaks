			...
			while(eventType != XmlPullParser.END_DOCUMENT) {
				...
				switch(eventType) {
					...
					case XmlPullParser.START_TAG:
						...
						// if <_switch>
						else if(tagName.equalsIgnoreCase("_switch")) {
							//Log.w("parseUIFromXml", "switch name = " + parser.getAttributeValue(null, "name"));
							//Log.w("parseUIFromXml", "switch description = " + parser.getAttributeValue(null, "description"));
							//Log.w("parseUIFromXml", "switch action = " + parser.getAttributeValue(null, "action"));
							//Log.w("parseUIFromXml", "switch label = " + parser.getAttributeValue(null, "label"));
							if (pane != null)
							{
								_switch = new SyhSwitch(this);
								_switch.name = parser.getAttributeValue(null, "name");
								_switch.description = parser.getAttributeValue(null, "description");
								_switch.action =parser.getAttributeValue(null, "action");
								_switch.label = parser.getAttributeValue(null, "label");
						    	pane.controls.add(_switch);								
							}
						}
						...
						else if(tagName.equalsIgnoreCase("spinnerz")) {
							//Log.w("parseUIFromXml", "spinner name = " + parser.getAttributeValue(null, "name"));
							//Log.w("parseUIFromXml", "spinner description = " + parser.getAttributeValue(null, "description"));
							//Log.w("parseUIFromXml", "spinner action = " + parser.getAttributeValue(null, "action"));
							if (pane != null)
							{
								spinnerz = new SyhSpinnerZ(this);
								spinnerz.name = parser.getAttributeValue(null, "name");
								spinnerz.description = parser.getAttributeValue(null, "description");
								spinnerz.action = parser.getAttributeValue(null, "action");
								pane.controls.add(spinnerz);							    	
							}
						}
						else if(tagName.equalsIgnoreCase("spinnerzItem")) {
							//Log.w("parseUIFromXml", "spinnerItem name = " + parser.getAttributeValue(null, "name"));
							//Log.w("parseUIFromXml", "spinnerItem value = " + parser.getAttributeValue(null, "value"));
							if (spinnerz != null)
							{
								spinnerz.addNameAndValue(parser.getAttributeValue(null, "name"), parser.getAttributeValue(null, "value"));
							}
						}
...
//    	handler.postDelayed(r, 5000);   	
    ...
    public boolean onCreateOptionsMenu(Menu menu) {
        //TODO: move SyhExtrasTab to options menu
    ...
    public boolean onOptionsItemSelected(MenuItem item) {
        ...
        switch (item.getItemId()) {
            ...
            case R.id.menu_restore:
            	Intent i = new Intent( MainActivity.this, ListProfiles.class );
	...
	protected void onActivityResult(int requestCode, int resultCode, Intent data) {
		if ( requestCode == REQUEST_TEXT ){
			if ( resultCode == Activity.RESULT_OK ){	
				...
				// Guardo el nombre del perfil seleccionado de ListProfiles
				String filename = data.getExtras().get("filename").toString();
				
				// Si el valor recibido es "restart" llamo de nuevo a ListProfiles
				if ("restart".equals(filename)){
					Intent i = new Intent( MainActivity.this, ListProfiles.class );
	                MainActivity.this.startActivityForResult(i, REQUEST_TEXT);
				}
				// Con cualquier otro valor cargo el perfil seleccionado
				else {
					Utils.executeRootCommandInThread("cp /res/customconfig/default.profile $PROFILE_PATH/default.profile");
	      	        ...
	      	        Utils.executeRootCommandInThread("/res/uci.sh select "+filename);
